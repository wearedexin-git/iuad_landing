...
export default defineConfig(({ command }) => ({
  ...
  base: command === 'build' ? '/landing/design-architettura-interni/' : '/',
  plugins: [
  ...
  server: {
    proxy: {
      ...
      '/landing/design-architettura-interni/submit.php': {
        target: 'http://localhost:8888',
        changeOrigin: true,
        rewrite: (path) => path.replace(/^\/landing\/design-architettura-interni/, ''),
      },
    },
  },
