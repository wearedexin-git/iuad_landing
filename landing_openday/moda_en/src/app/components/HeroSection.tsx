...
const HOW_YOU_KNOWS_OPTIONS = [
  ...
  { label: "School / careers advice", value: "5" },
  { label: "Other", value: "7" },
];
...
export function HeroSection({ onBookClick: _onBookClick }: { onBookClick: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validazione privacy
    if (!privacy) {
      setErrorMsg("You must accept the privacy notice to continue.");
      return;
    }
    ...
    if (!formData.comeConosciuto || formData.comeConosciuto === "") {
      setErrorMsg("Please tell us how you heard about IUAD.");
      return;
    }
    ...
    try {
      ...
      try {
        ...
        setErrorMsg(
          import.meta.env.DEV
            ? "The PHP server is not responding or did not return JSON. From the public folder, run: php -S localhost:8888 (same port as the proxy in vite.config.ts), then try submitting again."
          ...
          <div className="flex flex-col gap-6 items-start w-full">
            ...
            <div className="flex flex-col gap-4 items-start w-full">
              <p className={HERO_BODY_COPY_CLASS}>
                ...
                <span className="font-bold">three-year undergraduate programme</span>, speak
                with <span className="font-bold">current students</span>, and meet our{" "}
                <span className="font-bold">industry faculty.</span>
              </p>

              <div className="flex flex-col gap-6">
                {CAMPUSES.length === 0 ? (
                  <p className={HERO_BODY_COPY_CLASS}>
                    No campuses with active Open Day dates are configured. In{" "}
                    <span className="font-bold">openday-config.json</span> add at least one entry under{" "}
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-6">
            ...
            <div className="flex gap-3 items-start">
              ...
              <p className="font-sarabun font-light text-[#201f1f] text-[length:calc(13px-2pt)] md:text-[length:calc(14px-2pt)] leading-[1.5]">
                I confirm that I have read the privacy notice pursuant to Art. 13 of GDPR (EU) 2016/679 and I consent to the processing of my personal data to receive information about your initiatives by email and phone.
              </p>
            </div>
