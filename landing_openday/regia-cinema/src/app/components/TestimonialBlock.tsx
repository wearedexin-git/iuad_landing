...
function QuoteIcon() {
  return (
    <div className="w-[50px] h-[35px] md:w-[69px] md:h-[48px] shrink-0">
      ...
      >
        <path d={svgPaths.p1bfd52c0} fill="#801718" />
      </svg>
    </div>
  );
}
...
function TestimonialCard({ testimonial }: { testimonial: Testimonial }) {
  return (
    <div className="bg-white flex flex-col gap-4 p-8 md:p-10 rounded-3xl border-2 border-[#801718] w-full h-full">
      <QuoteIcon />
      <div className="flex flex-col gap-3">
        <div className="flex items-center gap-2">
          <RegiaIcon className="w-[42px] h-[42px] shrink-0" />
...
export function TestimonialBlock({ onBookClick }: { onBookClick: () => void }) {
  ...
  if (!isDesktop) {
    return (
      <section
        ...
        className="relative bg-[#fbdee6] py-16 overflow-hidden"
      >
        {/* Titolo animato */}
  ...
  return (
    <div
      id="testimonial"
      ref={sectionRef}
      className="relative bg-[#fbdee6] overflow-hidden"
      style={{ height: "100vh" }}
    >
      {/* Titolo animato */}
      <div
